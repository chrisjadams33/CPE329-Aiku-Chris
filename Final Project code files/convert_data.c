...
uint32_t convert_SPO(uint32_t calibrated_data){

    char tens_place = 0x00;
    char ones_place = 0x00;
    uint32_t num_return = 0;

    while(calibrated_data > 10){                   //compare the data(which is in uV) to 1V
        calibrated_data = calibrated_data - 10;
        tens_place += 1;                                //increment value of the ones place
    }
    while(calibrated_data > 1){                    //compare the data(which is in uV) to 0.1V
        calibrated_data = calibrated_data - 1;
        ones_place += 1;                              //increment value of the tenths place
    }
    if (calibrated_data >= 0.5)
        ones_place += 1;
